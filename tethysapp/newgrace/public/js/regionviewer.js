...
init_vars = function() {
    ...
    thredds_wms = thredds_wms.replace(/\/?$/, "/")
...
var signal_process = $("#select_signal_process")
    .find("option:selected")
    .val()
var storage_type = $("#select_storage_type")
    .find("option:selected")
    .val()
...
function addGraph() {
    ...
    var storage_type = $("#select_storage_type")
        .find("option:selected")
        .val()
    var signal_name = $("#select_signal_process")
        .find("option:selected")
        .text()
    var storage_name = $("#select_storage_type")
        .find("option:selected")
        .text()
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (var i = 0; i < length; i++) {
                ...
                if (isNaN(this_data)) this_data = null
                if (isNaN(this_depletion_data)) this_depletion_data = null
            ...
            myseries = {
                ...
                tooltip: {
                    ...
                    xDateFormat: "%A, %b %e, %Y",
                    headerFormat:
                        '<span style="font-size: 12px; font-weight:bold;">{point.key} (Click to visualize the map on this time)</span><br/>'
                }
            }
...
get_ts = function() {
    if ($("#point-lat-lon").val() == "") {
        $(".error").html(
            "<b>No feature selected. Please create a feature using the map interaction dropdown. Plot cannot be generated without a feature.</b>"
        )
        return false
    } else {
        $(".error").html("")
    ...
    var xhr = ajax_update_database("get-plot-reg-pt", {
        region_dis_name: region_dis_name,
    ...
    xhr.done(function(result) {
        if ("success" in result) {
            ...
            myotherchart = Highcharts.stockChart("plotter", {
                ...
                series: [
                    ...
                    },
                    {
                        ...
                        tooltip: {
                            valueDecimals: 2,
                            valueSuffix:
                                " Change in Volume since April 16, 2002 (Acre-ft)",
                            xDateFormat: "%A, %b %e, %Y",
                            headerFormat:
                                '<span style="font-size: 12px; font-weight:bold;">{point.key} (Click to visualize the map on this time)</span><br/>'
                        }
                ...
                lang: {
                    noData:
                        "There is no data to display.  Please select a point where data exists."
                },
                noData: {
                    style: {
                        fontWeight: "bold",
...
function updateWMS() {
    ...
    testLegend.onAdd = function(map) {
        var src =
            testWMS +
            "?REQUEST=GetLegendGraphic&LAYER=lwe_thickness&PALETTE=" +
            type +
            "&COLORSCALERANGE=" +
            colormin +
            "," +
